...
use aerospike::{
    as_bin, as_key, as_list, as_map, as_val, Bins, MapPolicy, MapReturnType, ReadPolicy, Record,
    Value, WritePolicy,
};
...
async fn map_operations() {
    ...
    let rec: Record<HashMap<String, Value>> = client.operate(&wpolicy, &key, &[op]).await.unwrap();
    ...
    let rec: Record<HashMap<String, Value>> = client.get(&rpolicy, &key, Bins::All).await.unwrap();
    assert_eq!(
    ...
    let rec: Record<HashMap<String, Value>> =
        client.operate(&wpolicy, &key, &[dec, inc]).await.unwrap();
    ...
    client
        .operate::<HashMap<String, Value>>(&wpolicy, &key, &[op])
        .await
        .unwrap();
